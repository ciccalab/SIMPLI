...
cell_population <- arguments[[5]]                                                                                          
markers <- strsplit(arguments[[6]], "@")[[1]]                                                                              
resolutions <- as.numeric(strsplit(arguments[[7]], "@")[[1]])                                                              
high_color <- arguments[[8]]                                                                                               
mid_color <- arguments[[9]]                                                                                                
low_color <- arguments[[10]]                                                                                                
output_folder <- arguments[[11]]   
...
lapply(resolutions, function(res){
	...
	# Boxplots by cluster
	if(n_categories == 2){
		boxplot_output_folder <- file.path(output_folder, "Boxplots", res, comparison_column)  
		...
		lapply(clusters, function(cluster){
			pdf_plotter(cluster_boxplots[[res]][[cluster]], filename =  paste0(boxplot_output_folder,
				"/boxplot_", comparison_column, "_", cluster,"_", res, ".pdf"))
		})
	}	
})
